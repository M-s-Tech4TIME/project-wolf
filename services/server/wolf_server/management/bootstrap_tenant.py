...
Per doc 05 §Tenant misconfiguration / onboarding and Phase 4 Slice 2:
...
re-run for an existing tenant slug refuses unless `--update` is passed,
...
`--skip-validation`. The tenant is provisioned with `validated_at=NULL`
...
from wolf_server.tenancy.context import VALID_ROLES
...
class ConnectionValidationError(RuntimeError):
    ...
    NOT persist a TenantWazuhConfig row when this is raised — doc 05
    §Tenant misconfiguration mandates "validate-before-persist."
    """
...
async def _validate_wazuh_connection(
    *,
    ...
    must succeed before the tenant is persisted (Wazuh's Indexer and
    ...
    async with httpx.AsyncClient(verify=verify_tls, timeout=timeout) as client:
        ...
        try:
            ...
            raise ConnectionValidationError(
                f"Indexer at {opensearch_url} is unreachable: "
        ...
        try:
            ...
            raise ConnectionValidationError(
                f"Server API at {server_api_url} is unreachable: "
                f"{type(exc).__name__}: {exc}"
            ) from exc
        if response.status_code == 401:
            raise ConnectionValidationError(
...
class TenantAlreadyExistsError(RuntimeError):
    ...
    Doc 05 §Tenant misconfiguration says connection profiles are
...
async def _tenant_exists_with_validated_config(
    db: AsyncSession, tenant_slug: str
) -> tuple[bool, bool]:
    """Returns (tenant_exists, has_validated_wazuh_config).
    ...
    if tenant is None:
        return False, False
    cfg = await db.scalar(
        select(TenantWazuhConfig).where(TenantWazuhConfig.tenant_id == tenant.id)
    )
...
async def bootstrap_tenant(
    *,
    ...
    """Bootstrap or update a tenant.

    First-time call: validates the Wazuh connection (or skips via
    `skip_validation=True`), then provisions the tenant + admin + Wazuh
    config + secrets.

    Re-run for an existing tenant slug: refuses with
    TenantAlreadyExistsError unless `update=True`. With update=True,
    re-validates and overwrites the existing config + credentials. The
    tenant + admin-user rows are kept (preserves user↔tenant bindings)
    ...
    # First check: is this a re-run on an existing tenant?
    async with factory() as db:
        exists, has_validated_config = await _tenant_exists_with_validated_config(
            db, tenant_slug
        )
    if exists and has_validated_config and not update:
        await engine.dispose()
        raise TenantAlreadyExistsError(
            f"Tenant slug={tenant_slug!r} already exists with a validated "
            f"Wazuh config (per doc 05 §Tenant misconfiguration, validated "
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Re-validate and overwrite an existing tenant's Wazuh config + "
            "credentials. Required when re-running this CLI for an already-"
            "validated tenant slug; doc 05 §Tenant misconfiguration treats "
    ...
    p.add_argument(
        ...
        help=(
            ...
            "yet' placeholder pattern in ONBOARDING §3.9 — the tenant is "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        sys.stderr.write(
            ...
            f"tenant will have validated_at=NULL and tool calls hitting "
