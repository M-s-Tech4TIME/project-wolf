"""Wolf orchestrator launcher — Phase 5.4-c.

Wraps `uvicorn` so the orchestrator listens HTTPS when its TLS cert
...
`.local/certs/orchestrator/`, the next `python -m app` start picks
them up, and the orchestrator flips to HTTPS with no env edits.
`wolf-cert revoke` deletes them; the launcher drops back to HTTP.

Two invocation forms supported:

    python -m app          # what docs/restart.md now recommends
    uv run python -m app   # same, via uv's venv resolution

The historic `uvicorn app.main:app --host 0.0.0.0 --port 8000`
...
both `http://` and `https://` schemes, so flipping the orchestrator
to HTTPS doesn't require a CORS edit.
"""
...
def main() -> None:
    ...
    print(
        f"wolf-orchestrator: serving {scheme}://"
