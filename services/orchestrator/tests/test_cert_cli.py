...
def test_init_creates_ca_and_two_leaves(tmp_path: Path) -> None:
    ...
    assert store.leaf_cert_path("orchestrator").exists()
    assert store.leaf_key_path("orchestrator").exists()
    assert store.leaf_cert_path("frontend").exists()
    assert store.leaf_key_path("frontend").exists()
...
def test_init_applies_custom_san_dns_and_ip(tmp_path: Path) -> None:
    ...
    store = CertStore(root=tmp_path)
    leaf = read_cert_pem(store.leaf_cert_path("orchestrator"))
    s = cert_status(leaf)
...
def test_init_leaves_have_strict_key_permissions(tmp_path: Path) -> None:
    ...
    for path in [
        store.ca_key_path,
        store.leaf_key_path("orchestrator"),
        store.leaf_key_path("frontend"),
    ]:
...
def test_status_prints_ca_and_leaves(
    ...
    assert "leaf 'orchestrator'" in out
    assert "leaf 'frontend'" in out
...
def test_add_host_appends_dns_san_to_orchestrator(tmp_path: Path) -> None:
    cmd_init(_ns(
        cert_dir=str(tmp_path), years=100,
        ca_cn="CA", ca_org="Org", san_dns=[], san_ip=[],
    ))
    ...
    assert rc == _ExitCode.OK
    orch = cert_status(read_cert_pem(store.leaf_cert_path("orchestrator")))
    front = cert_status(read_cert_pem(store.leaf_cert_path("frontend")))
    assert "wolf.example.org" in orch.san_dns
    # `--leaf=orchestrator` means the frontend was NOT touched.
    assert "wolf.example.org" not in front.san_dns
...
def test_renew_reissues_leaves_and_extends_not_after(tmp_path: Path) -> None:
    ...
    store = CertStore(root=tmp_path)
    before = cert_status(read_cert_pem(store.leaf_cert_path("orchestrator")))
    ...
    assert rc == _ExitCode.OK
    after = cert_status(read_cert_pem(store.leaf_cert_path("orchestrator")))
...
def test_renew_can_reissue_the_ca(tmp_path: Path) -> None:
    ...
    leaf = cert_status(read_cert_pem(store.leaf_cert_path("orchestrator")))
