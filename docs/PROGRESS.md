...
**Last updated:** 2026-06-16 by claude-code (**6.6-c SHIPPED** — per-org Wazuh credentials backend (ADR 0020), the second backend layer of Phase 6.6 (Superuser-owned Wazuh component mapping). The Superuser configures, PER ORG, the Wazuh credentials that org uses to query the install ecosystem (URLs come from 6.6-a's install topology). New `wolf_server/wazuh/credentials.py`: `probe_org_credentials` (reuses 6.6-a's `probe_indexer`/`probe_manager_api` for auth + adds a SCOPE SUMMARY — authenticate→JWT→`/agents?limit=1`+`/groups?limit=1` total_affected_items, best-effort/graceful, never raises) per ADR's "Test credentials" contract; `resolve_endpoints_from_topology` → (indexer_url, manager_url, verify_tls) (first indexer node + manager master for distributed; random per-query routing is 6.6-e). New `wolf_server/api/wazuh_credentials.py` router: `GET`/`PUT /api/v1/superuser/organizations/{id}/wazuh-credentials` (Superuser-only via `require_superuser` — org Admin/Engineer rejected at the dependency). PUT = SOFT-FAIL save (ADR decision 3): credentials persist even when the probe fails (`validated_at` stays null), so the Superuser can save before the Wazuh-side user is provisioned; warning + scope ride back. URLs come from the install topology → PUT with NO configured topology = 409 ("configure the Wazuh ecosystem first"). Audit `organization.wazuh_credentials.updated` (org-scoped, NEVER logs creds); "omit password ⇒ keep existing" (422 on first save). Migration 0012 adds optional `wazuh_agent_groups` (JSONB nullable, additive). Router mounted in main.py. THREE decisions (flagged): (1) TWO credential pairs per org (Indexer + Server-API), NOT the ADR's literal single `wazuh_api_user` — Wazuh separates those auth backends + the existing model/bootstrap already do two; collapsing breaks the separation. (2) COHERENCE BRIDGE — the per-org row keeps its URL columns, SOURCED FROM THE INSTALL TOPOLOGY on save, so the current runtime resolver is untouched until 6.6-e reads the topology fresh per query + drops them (so 6.6-c is additive, not destructive). Transitional caveat: a per-org row's cached URLs go stale if topology changes before re-save — exactly what 6.6-e's read-fresh fixes. (3) PUT requires a topology first (409); soft-fail applies to the probe, not this prerequisite. Gate: ruff + mypy --strict (`wolf_server/wazuh`+`wolf_server/api` already in strict set — NO CI change) + **490 backend / 0 skip / 0 warning** (was 476; +14 — 2 probe/scope via httpx.MockTransport, 2 topology-resolver pure, 10 API) + cross-org isolation (18); `alembic check` clean ("No new upgrade operations detected") + 0012 round-trips on Postgres; no new dep (dep-audit unaffected). NO CI workflow change needed. DEFERRED operator web-test (tracked, NOT skipped): live functional test (real per-org creds via GUI + real probe + scope) lands with 6.6-d (UI)/6.6-e (runtime), needs real Wazuh. NOTE retained: torch (opt-in embeddings-local extra) CVE-2025-3000, no fix — out of CI dep-audit scope. NEXT: 6.6-b (install-level Wazuh Ecosystem UI) / 6.6-d (per-org creds UI) / 6.6-e (runtime resolution) per operator.)
