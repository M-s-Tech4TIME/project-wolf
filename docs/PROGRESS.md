...
**Last updated:** 2026-06-16 by claude-code (6.5-h SHIPPED — invite-link verification flow (ADR 0018 item 9, SPLIT — same-network gate → 6.5-h.2), operator web-tested + headless-Chrome verified. Admin-created accounts start UNVERIFIED with a single-use 7-day invite token (SHA-256 hash stored, never the raw token); the user pastes the invite link after login to flip to VERIFIED. Migration 0010 (User.verification_status + verification_token_hash + verification_token_expires_at; backfills existing rows → verified so nobody's locked out). Verification GATE in require_organization_context (zero extra query — binding.user already eager-loaded), the chokepoint for all org data; /me + /me/organizations + verify-invite + logout stay reachable; bootstrap/recovery/Superuser created verified (4 User() sites). New auth/invite.py (mint/hash/verify); create_member returns raw token ONCE beside the one-time password; POST /organization/users/{id}/regenerate-invite-link (Admin, recover lost link, old token invalidated, 409 if verified); authenticated POST /auth/verify-invite (single-use, 403 missing/expired/mismatch WITHOUT consuming, 409 already-verified; `# same-network gate goes here` marks the 6.5-h.2 hook). MeResponse + LoginResponse carry verification_status. Frontend: /verify paste-link screen; routing guards send unverified non-superusers to /verify (login routes DIRECTLY — no /chat hop; chat layout redirects as defense-in-depth); Users-page Verified/Unverified badge + invite expiry + "Generate invite link" action; invite link revealed once on create/regen. Dialog primitive hardened (frosted backdrop matching chats overlay, scroll-safe so footers don't escape, wider sm:max-w-2xl invite modals, full link wrapped break-all) — operator UI/UX pass. Audit (isolated from notifications): …invite_generated / …invite_regenerated / auth.invite_verification.succeeded|failed{reason}. 431 backend + cross-org + mypy --strict green; alembic check clean (0010 round-trips on Postgres); frontend tsc/eslint(0)/build green; headless-Chrome confirmed login→/verify + /chat→/verify for unverified users. No CI workflow change needed. LESSON: `npm run build` then restarting next dev serves a STALE bundle — clear .next while stopped before restarting dev (memory updated). NEXT: 6.5-h.2 (same-network gate, needs custom dashboard server) or 6.6 per ordering. SMTP service (system email) planned as a future phase — discussion in progress.)
