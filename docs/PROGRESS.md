...
**Last updated:** 2026-07-12 by claude-code (**PROJECT WRAP-UP — development paused indefinitely (operator's subscription ending).** Everything through Phase 6-f is SHIPPED and CI-green; nothing is half-finished. The two sessions since the previous header (2026-07-11/12, full CHANGELOG entries exist): **PostgreSQL 17→18 full replacement** (code gate `REQUIRED_MAJOR_VERSION=18` rejects 17, pgdg `postgresql-18-pgvector`, `pgvector/pgvector:pg18` CI images, live dev cluster `pg_upgradecluster`'d to 18.4 + pgvector 0.8.5 on :5432, PG17 removed) and **ADR 0033 — fully configurable embedding stack** (every knob per-embedder: dimension/MRL/prefixes/num_ctx/char-limits, DB columns follow settings via resumable `embedding_schema --apply`, **no-cap 4096 dims** via binary-quantized HNSW + exact-cosine rerank, `embedding_bench` harness; live machine = the measured-best config: qwen3-embedding native-4096 primary + nomic-embed-text-v2-moe 768 aux — combo MRR@10 **0.963** vs 0.766 for the nomic combo; three real bugs found+root-fixed during the switch: NOT-NULL stamp sentinel, prefix `\n` normalizer, BQ `hnsw.ef_search` cap). **THE WRAP-UP ITSELF:** NEW `docs/HANDOVER.md` (state + queue + standing-rules digest + credentials + environment rebuild), NEW `docs/CLAUDE-RESUME-PROMPT.md` (paste into a fresh Claude Code session to resume cold), NEW tracked `credentials.example/` (CHANGE_ME templates of all four credential files; real `credentials/` stays gitignored — public repo), ONBOARDING resume-pointer + embedding-model pulls, MEMORY.md index trimmed under its load limit. GATES at pause (fresh 2026-07-12): **963 backend tests / 0 skips**, mypy --strict Success (117 files), ruff clean. **RESUME HERE → `docs/HANDOVER.md`**; queue = (1) 6-f.4 operator web-test (virustotal upsert; wazuh-wui cred + qwen3:8b) → (2) Nemotron model-switch eval (operator-gated) → (3) Phase 6.9→6.7→6.8 → (4) 6.10 config-settings → (5) 6.11/6.12 → (6) 6.13 grounding enrichment → Phase 7+.)

_Prior:_ 2026-07-07 (**6-f.6 — deployment-aware config application; slice SHIPPED (the third 6-f.4 web-test directive).** A config change is now applied per DEPLOYMENT TYPE. GROUNDED live 2026-07-07 on the operator's 3-node cluster (read-only probe via wolf-server's own resolver): Wazuh does NOT sync `ossec.conf` — the nodes genuinely diverge (master 5 integrations / workers 4) — so 6-e.4/6-f.4's `PUT /manager/configuration` (master only) left workers stale; the per-node endpoints exist (`GET/PUT /cluster/{node}/configuration`, RBAC `cluster:update_config`) and ONE `GET /cluster/configuration/validation` validates all nodes (per-node `.../validation` is 404). NEW `wazuh/cluster.py`: `get_cluster_nodes` (status→nodes master-first; `[]`=all-in-one; RAISES on unusable inventory — no silent master-only fallback) + path-safe node names. NEW `ACTION_UPDATE_CLUSTER_CONFIG` + `WazuhServerApiActionClient.update_node_configuration`. **Propose tool** detects at propose time; for a cluster captures+dry-runs per node and freezes `deployment="cluster"`+`node_current_contents`; scope respects INSTANCE LOCALITY (single-instance section = uniform cluster-wide; repeated instance = only nodes that carry it; upsert present nowhere = cluster-wide ADD); preview names in-scope+untouched nodes; distributed apply needs `cluster:update_config` (fail closed). **Executor** branches on `deployment`: cluster path snapshots each in-scope node's whole file (`prior_state.kind="cluster_configuration"`), applies `build_candidate` per node, writes each, validates cluster once, proves persistence per node, restarts ONCE — ALL-OR-NOTHING (any failure restores every written node); freshness re-detects cluster+per-node staleness; reverse restores each node's snapshot hash-verified; all-in-one path unchanged (staleness message now names the file/node). **GUI** `scopeSummary` renders "…in N cluster node(s) (…)" vs "…ossec.conf". Prompt #4: one-line "Wolf handles the deployment automatically + states the in-scope nodes". HONEST BOUNDARY: indexer/dashboard config files are unreachable via the Server API → wolf-pack (Phase 12), stated not faked. GATES: **924 backend tests / 0 skips** (+16: 7 cluster.py, 6 distributed executor, 3 distributed propose), `make typecheck` Success (117 files), ruff clean, dashboard tsc+eslint clean. ADR 0032 addendum (2026-07-06). NEXT: **operator web-test of 6-f.6** on the live cluster (a distributed change → verify all relevant nodes updated + one restart; undo restores each node; needs the `manager:update_config`+`cluster:update_config` admin cred + a loop-capable model) → then the operator-requested **Nemotron 3 Ultra/Super (free) model-switch evaluation** ([[model-switch-nemotron-after-slices]]); Phase 6.13 grounding enrichment queued "later".)

_Prior:_ 2026-07-06 (**6-f.5 — unbounded persistence + any-unique-field disambiguation; slice SHIPPED (the 6-f.4 web-test feedback fixes).** From the operator's web-test (duplicate `custom-tracecat` + a generic request dying at 8 steps). **(1) NO hard step caps** (operator directive: "utilize the step count, but never limit the step count to any specific value"): `agent/loop.py` dropped `for step in range(budget)` — the loop persists until the model answers; the remaining stops are REAL: the no-progress guard (unchanged mechanism, now the primary guard), the NEW **context-fit guard** (last call's `input_tokens` ≥ `AGENT_CONTEXT_FIT_THRESHOLD` 0.8 × the provider's EFFECTIVE window — new `effective_context_window()`: Ollama = its loaded `num_ctx`, FailoverProvider = min across links; stops BEFORE Ollama's silent head-truncation), and the NEW optional `AGENT_STEP_BREAKER` circuit breaker (default 0 = off; Phase 6.10 GUI consumer). EVERY forced stop → `_synthesized_stop` (no-tools re-prompt composing the best answer from gathered evidence, honest about gaps) — **`stop_reason="budget_exhausted"` + the canned apology RETIRED** (TS union keeps the member for legacy persisted rows; dashboard needed no functional change — no-budget step phrases already existed). `max_safe_autonomous_steps` = a soft take-stock checkpoint cadence (`_CHECKPOINT_NUDGE` every N steps). Web-research budget 12 → 32. `SYSTEM_PROMPT` gained **#7 PERSIST UNTIL SATISFIED** (dynamic think/reason/verify/justify/validate on ANY request shape — generic/partial/full; act on tool guidance instead of giving up); GUIDED/WEB suffixes de-budgeted. Discovery: the live 8-step wall was the failover chain's conservative floor (min across links → qwen3:8b's 8), not cohere's grade (15). **(2) Any-unique-field disambiguation** (the duplicate-name fix): `block_key` matches the identity element FIRST and falls back to ANY leaf value uniquely selecting one live instance (`<hook_url>`/`<api_key>`); ONE shared `_identified_matches` selector behind `find_identified_blocks` → tool capture, `build_candidate`, executor and `block_persisted`/`block_removed` all agree; B2 ops now valid on ANY unblocked section (validator's IDENTITY_KEYS-membership requirement dropped); identity-carry → `content_carries_key` (identity OR carried leaf value; to change the addressing field itself, address by another unique field); ambiguous-key refusals **ENUMERATE each instance's discriminating fields** (`describe_instances` — values no other instance shares) + "re-call with one" — the refusal now TEACHES the fix (the web-test hallucination was caused by the starved refusal: Wolf had TRIED `block_key=hook_url`, and name-only matching rejected it). Truly indistinguishable duplicates stay hand-fix. ADR 0032 addendum (2026-07-06) records both. GATES: **908 backend tests / 0 skips** (+12), `make typecheck` Success (116 files), ruff clean, dashboard tsc+eslint clean. (Repo-wide `ruff format` drift ~47 files pre-exists; CI gates `ruff check` only — formatted only touched files.) NEXT: **operator web-test of 6-f.5** (rerun both tracecat scenarios; needs the `manager:update_config` credential + a loop-capable chat model — qwen3:8b) → **6-f.6 deployment-aware config application** (detect via `/cluster/status`; per-node `/cluster/{node_id}/configuration` VERIFIED EMPIRICALLY first — ossec.conf is NOT cluster-synced, today's PUT writes the master only; indexer/dashboard config = wolf-pack Phase 12 boundary, stated honestly) → then the operator-requested Nemotron 3 Ultra/Super (free) model-switch evaluation.)
...
_Prior:_ 2026-07-05 (**6-f.3 — web_search / web_fetch / web_crawl LIVE: full A6 security, docs-first ranking, citations in the evidence panel; slice 6-f.3 CLOSED.** NEW `research/` request path: `weburl.py` (**SSRF guard** — http/https only, creds-in-URL rejected, punycode host normalization; EVERY resolved address vetted (loopback/RFC-1918/link-local incl. 169.254.169.254/ULA incl. fd00:ec2::254/IPv4-mapped-v6/multicast — CPython gotcha: v4 multicast is `is_global=True`, rejected explicitly)/reserved/unspecified; connection **PINNED to the vetted IP** with hostname in Host + TLS SNI so cert verification still runs — DNS-rebinding defeated; redirects re-validated per hop, 5-hop cap) · `extract.py` (stdlib-only HTML→title/text/links, chrome stripped, control chars sanitized §12) · `fetcher.py` (one guarded fetch path: DECOMPRESSED byte-cap streaming abort §4, text-ish content-type enforcement, whole-fetch deadline §14, honest UA §11, injectable client+resolver) · `policy.py` (docs-first tiers official_docs/official/official_github(path-aware)/community; suffix-anchored matching defeats `wazuh.com.evil.com`; blocklist ships EMPTY — operator curation → Phase 6.10; stdlib eTLD+1 approximation, no tldextract) · `crawl.py` (BoundedCrawler: robots.txt per host fail-open convention, sitemap-first via regex `<loc>` scan — entity-bomb-immune by construction, same-registrable-domain, per-host politeness delay, best-first query-scored frontier with seed always first, depth/page caps + 120s crawl deadline) · `context.py` (per-request `ResearchContext` = provider+fetcher+config snapshot+**web budget**; `open_research_context` async CM owns client lifecycles, None when disabled — per-request isolation per grounding-concurrency-model). THREE `ReadTool`s (`tools/web_research.py`): docs-first-re-ranked `web_search` w/ one Citation PER result; `web_fetch` w/ **untrusted-content envelope** (16K cap §2/§5); `web_crawl` w/ enveloped 3K excerpts + honest bookkeeping — model inputs narrow, NEVER widen, server caps. WIRING: registration gated on `WEB_SEARCH_ENABLED` (14 tools on / 11 off; reachability = CALL-time degradation, no boot-order coupling — deliberate A1 refinement, ADR 0016); `WEB_RESEARCH_SUFFIX` system-prompt section rides the same gate (query-egress minimisation taught §3); dispatcher: `research` slot + NEW `ToolDegradedError` branch (`tool.call.degraded` audit — expected failures degrade cleanly, WolfErrors still re-raise, budget exhaustion = `GuardrailViolation`); loop collects plural `citations`; both chat endpoints open the context. `Citation` + GUI type gained url/title/source; **evidence panel renders clickable links + official-source badges** (A5). CONFIG (A7): 7 knobs — search max_results 8 / budget-per-request 12 / fetch max_bytes 2MB decompressed / fetch timeout 20s / crawl depth 2 / pages 12 / per-host rate 1s — all Phase 6.10 GUI consumers. TESTS: +68 hermetic (83 web-research; MockTransport + injected resolver, zero live DNS/HTTP) — full SSRF matrix, redirect-bounce, byte-cap, content-type, robots/sitemap/domain/depth/cap crawler bounds, ranking, envelope, budget, degradation, gating; **866 passed / 0 skips**; ruff+format, mypy --strict (116 files), dashboard tsc+eslint green. **LIVE self-validated**: flag on + restart → qwen3:8b chained 1 search + 3 fetches unprompted, documentation.wazuh.com organic #1 (official_docs), cited official-docs answer in 53s. `.env` has `WEB_SEARCH_ENABLED=1` — READY for the operator web-test (docs-first + citations + a bounded doc-topic crawl). ADR 0032 addendum records every refinement. NEXT: **6-f.4** — config-authoring generalization (research → confirm-diff → dry-run-validate → propose; block-identity for repeated `<integration>`); web-test virustotal end-to-end.)

_Prior:_ **2026-07-03** (**6-f.2 PACKAGING HALF — wolf-search.deb; slice 6-f.2 CLOSED.** `debian/wolf-search.*` mirroring wolf-database: `Architecture: all`, Depends = the official SearXNG apt list, .deb carries ONLY Wolf-owned artifacts (wrapper + settings/uwsgi templates + dedicated unit); **postinst = the exact 6-f.2 host recipe pinned to `747cec4c`** (searxng user → clone + detached checkout at the pin → searx-pyenv + official pip sequence + PEP-517 editable install → settings template + per-host openssl secret install-once root:searxng 640 → ini to apps-available NEVER apps-enabled); requires network at install (github.com+PyPI) — ratified ADR 0032 trade-off, air-gapped skips it (`Recommends: wolf-search` in the `wolf` meta-package, NOT Depends). Upgrades incremental (postrm keeps the checkout across upgrades; remove deletes /usr/local/searxng, /etc/searxng preserved until purge; searxng user never removed). CI: smoke-deb + release.yml four→FIVE .debs; **smoke-deb-install installs wolf-search on the clean runner, asserts every postinst effect (pin/venv/640 secret/no apps-enabled) then STARTS the service + runs `wolf-search health` end-to-end** (only component runnable without operator env; timeout 15→25); make smoke-systemd covers the 4th unit template. Docs: RELEASING (5 debs/12 assets), docs/17, ONBOARDING (topology + sideload cmd), roadmap 6-f.2 ✅, ADR 0032 addendum + Appendix A → real postinst. NEXT: **6-f.3** — web_search/web_fetch/bounded web_crawl tools + full A6 security + docs-first + citations, registration gated on WEB_SEARCH_ENABLED; web-test docs-first + citations.)

_Prior:_ **2026-07-03** (**6-f.2 HOST HALF — wolf-search LIVE on 127.0.0.1:1307 (official SearXNG install, dedicated unit, health check healthy).** Installed strictly per the OFFICIAL step-by-step docs (operator directive): official apt list, `searxng` system user, clone → **pinned commit `747cec4c` = 2026.7.3**, venv + PEP-517 editable install, official template settings.yml + openssl secret, official check run (HTTP 200). BOTH predicted conflicts REPRODUCED live: limiter:true w/o valkey → startup ERROR; html-only formats → format=json 403. FOUR minimal deltas (rest stays official-default; fine-tune pass later): port **1307** (operator-chosen) + explicit loopback bind · formats +json (the adapter wire format) · limiter:false + valkey block removed. Service = official Debian uWSGI ini VERBATIM at the official path with ONE deviation (http-socket 127.0.0.1:1307 — no nginx; NOT symlinked into apps-enabled so distro uwsgi never double-runs) run by dedicated **wolf-search.service** (operator-confirmed; mirrors wolf-database ownership; uWSGI drops to searxng — verified in ps; journal clean; restart-resilient). Granian REJECTED for native (officially container-only → uWSGI is the by-the-book native runner). **Empirical adapter verification (closes the 6-f.1 deferral): unmodified SearxngProvider parsed the live JSON — 5/5 hits, documentation.wazuh.com organic #1** for a Wazuh query. `deploy/bin/wolf-search` wrapper (health/status/version/logs/start-stop-restart; `wolf-search health` probes the exact adapter endpoint, 403→settings-hint; renamed from `doctor` same-day by operator request) → `/usr/local/bin`; **health check healthy**. Hardening: settings.yml root:searxng 640 (holds secret_key). Repo: deploy/searxng/{settings.yml template (placeholder secret), searxng-uwsgi.ini}, deploy/systemd/system/wolf-search.service; `Settings.searxng_url` default → 1307 (+ default-pin test). ADR 0032 addendum (official layout supersedes Appendix-A sketch; 1307; uWSGI rationale). SUDO HYGIENE: operator's offered password DECLINED (would persist in transcripts); temporary NOPASSWD drop-in, every privileged block announced with reason, rule REMOVED + verified gone when host work ended. NEXT: **6-f.2 packaging half** — debian/wolf-search.* (postinst = this recipe pinned to 747cec4c), Recommends in the `wolf` meta-package, deb-smoke four→five .debs.)

_Prior:_ 2026-07-03 (**6-f.1 — research/ scaffolding: SearchProvider protocol + SearXNG adapter + resolver (ADR 0032's first implementation slice).** Opens Phase **6-f** (web research + config-authoring generalization; ADR 0032 accepted this same day, with the 6-e.4 config fixes shipped + web-test re-run PASSED first). NEW `wolf_server/research/` mirroring the model-provider abstraction: `interface.py` (`SearchResult` url/title/snippet/engine/published; runtime-checkable `SearchProvider.search(query, *, max_results)`; `SearchProviderError` 502 / `SearchProviderUnconfiguredError` 500; page-FETCH deliberately NOT on the protocol — provider-independent, lands with the 6-f.3 tools); `searxng.py` (`SearxngProvider`: GET `{SEARXNG_URL}/search?q=…&format=json`, **schema-validated** parse per ADR 0032 A6 §4 — pydantic top-level + per-entry, entries missing url/title DROPPED never guessed, `publishedDate` alias, non-200 raises with the enable-json-formats operator hint, unreachable/malformed → `SearchProviderError`; injectable httpx client = the OllamaAdapter stub pattern); `registry.py` (`get_search_provider_for_organization(ctx, settings, secrets)` mirroring `get_model_for_organization` — per-org seam + secrets RESERVED for Brave/Tavily later; **fails closed** when disabled; deferred-hosted vs unknown-provider errors distinct). Config seam: `WEB_SEARCH_ENABLED` (default **OFF** — stock installs never advertise web tools), `WEB_SEARCH_PROVIDER=searxng`, `SEARXNG_URL=http://127.0.0.1:8888` (wolf-search = wolf-server's SIDECAR, loopback in every recommended topology; dedicated tier swaps the URL — same seam as DATABASE_URL/OLLAMA_BASE_URL). INERT at runtime (nothing imports research/ until 6-f.3 registers the tools). Tests +15 (hermetic MockTransport — no live SearXNG; normalization/cap/drop/hints/fail-closed/deferred-vs-unknown; disabled-default pinned via `Settings.model_fields`, env-proof against the 6-f.3 `.env` flip). GATE: ruff + mypy --strict (research/ ADDED to ci.yml strict set) + **798 backend tests / 0 skip**; fixed Makefile typecheck DRIFT (was missing gateway/grounding/tools vs CI — now parity incl. research). NO migration; no runtime wiring. NEXT: **6-f.2** — the `wolf-search` Debian package (native-venv SearXNG mirroring wolf-database, unit + postinst + shell-wrapper), stand up on the host, empirically re-verify the live JSON shape.)

_Prior:_ 2026-07-02 (**6-e.4 — config_change: edit ossec.conf (section-scoped) + snapshot-restore reversal. CLOSES Phase 6-e (all four ADR-0025 action classes shipped).** The last + highest-blast-radius class, on the same per-class registry (validator/severity/executor/reversal), capability-gated + reversal-aware. v1 is the tightest gate of any class: **section-scoped, allowlisted, single-instance edits only** (`update_section` on `alerts`/`logging`/`remote`/`rootcheck`/`sca`/`syscheck`/`vulnerability-detection`) — break-the-manager sections (cluster/auth/indexer/ruleset) + repeated-in-stock sections (global ×2, localfile ×8, …) refused with guided messages (propose tool AND executor freshness enforce exactly-one live occurrence). **Diff-at-propose**: current section content captured into the proposal → approver reviews exact old→new; executor freshness refuses a STALE proposal (live section drifted). **Superuser-scoped** on `manager:update_config` (live probe: per-org `wolf-beta` holds `manager:read` but NOT the write; admin grants on `*:*:*`). **Snapshot-restore reversal** (reuses migration-0017 `prior_state`): forward captures the whole ossec.conf; reverse PUTs it back + **hash-verifies** → flips original to `rolled_back`. **Apply + authoritative confirm** (the 6-e.3 lesson): snapshot→replace one section→PUT→`/manager/configuration/validation`→auto-rollback if invalid→**re-read + confirm block persisted** (else restore+fail)→cluster restart (~18s). Files: `wazuh/config_change.py` (NEW domain, string-based section edit — ossec.conf is multi-root), `capabilities.py` (`manager:update_config` + `config_change`→`{manager:update_config}`, `RESOURCE_ANY`), `server_api.py` (`update_manager_configuration` bounded raw PUT), `validator.py`/`proposals.py` (`_validate_config_change` + severity HIGH), `executors.py` (`_ConfigChangeExecutor` + `ConfigValidationError`), `tools/propose_config_change.py` (NEW, registered), `prompts.py` #4, dashboard `app/actions/page.tsx` (target + `ConfigDiff` current→proposed + result rendering). GATE: ruff + mypy --strict clean; dashboard tsc+eslint clean; full backend **771 passed / 0 skip** (+31: domain/validator/severity 16, propose 6, execution 7 incl. auto-rollback/no-persist/stale/repeated-section/reverse-hash-verify, capability class-mapping, cross-org isolation); **NO migration** (reuses 0017); NO CI change (new modules under already-strict `wazuh`/`tools`/`gateway`; write-boundary grep already lists `execute_config_change`/`WazuhServerApiActionClient` → passes). ADR 0029 §7 + CHANGELOG + roadmap updated. **LIVE WEB-TEST PENDING** (operator-cleared, Superuser/admin credential — a REAL manager-global write): as a per-org user, ask to change a config section → REFUSED (no `manager:update_config`); as Superuser, update e.g. `<sca>` → /actions shows the current→proposed diff → approve → applied (validate→restart→verify); ask to restore → recalls the reason + restores ossec.conf + flips original to rolled_back. Restart wolf-server via `systemctl --user restart wolf-server.service`. NEXT (operator's call): Phase 6.10 config-settings system, or 6.9→6.7→6.8 notifications+SMTP, or per-org OpenRouter config.)

_Prior:_ 2026-06-30 (**6-e.3 FIX — authoritative rule_tuning verify (no phantom "succeeded").** First live web-test surfaced a real bug: disabling rule 100001 reported "succeeded" but the rule was NOT disabled (DB: `effective_level:5`, `matches:False`). A controlled live probe (clean rule 100700, adjust 5→7→restore) proved the ground truth: the write mechanism WORKS (PUT persists, validation passes, **cluster restart reloads in ~18s**, restore works) — `GET /rules` reflects the **on-disk file immediately** and returns the original AND the `overwrite="yes"` entry as SEPARATE items, so the old verify's `items[0]` grabbed the ORIGINAL (old level) → phantom "succeeded, matches:False." FIX (`gateway/executors.py`): **authoritative confirm** — after PUT+validate, re-read `local_rules.xml` and confirm OUR marked override block persisted (`rule_tuning.has_override`, NEW) BEFORE restart; restore+raise if not (honest failure). Verify no longer re-reads post-restart (races the restart API outage); surfaces pre-restart evidence (`override_written`/`levels`/`target_level_in_ruleset`); reverse confirms override REMOVED. `_resolve_rule` prefers the `local_rules.xml` entry (dual-file rule ids — 100001 existed in modesecurity_rules.xml @10 AND local_rules.xml @5). GUI `resultDetail` now renders rule_tuning evidence (was silently unrendered — only handled AR fields). Tests: shared-`_Disk` stubs + `test_forward_fails_honestly_if_override_does_not_persist` (the exact bug) + `has_override`. GATE: ruff + mypy --strict + dashboard tsc/eslint clean; full suite green. ADR 0029 §6 + CHANGELOG updated. Underlying write+restart was sound; the defect was verification+evidence. NEXT: operator re-tests Test B (should now show the override + evidence), then **6-e.4** (config_change).)

_Prior:_ **OR.1 — OpenRouter as a SELECTABLE model provider (chat + grounding).** ADR 0030. Operator wants free open frontier models (`nvidia/nemotron-3-ultra-550b-a55b:free` + `openrouter/owl-alpha`, both selectable) as a CONFIGURABLE OPTION — local Ollama stays the default (free + uncapped + on-prem). **Empirically verified live** (operator's key): both models exist, native tool-calling + ~1M ctx, $0; blocking chat + SSE streaming + **streamed tool-calling** confirmed end-to-end. Honest findings surfaced: free≠uncapped (all `:free` share a ~50/day cap; only Ollama is free+uncapped), free models may log prompts, embeddings reachable (2048-dim, hidden `/embeddings`) but need a 768→2048 migration → OR.2. `models/openai.py` (the shared OpenAI-PROTOCOL adapter — kept its name; serves OpenAI/Azure/vLLM/OpenRouter) gained real `chat_stream` (SSE deltas + index-keyed tool-call accumulation) + **429/error handling** (`ModelProviderRateLimitError`) + `extra_headers`. `models/openrouter.py` (NEW): thin `OpenRouterAdapter` pinning base + `HTTP-Referer`/`X-Title` + provider. `interface.py`: both models in `KNOWN_MODELS`. `model_resolver.py`: `openrouter` factory case; `config.py`: `openrouter_base_url`/referer/title. Key stored in secrets (`model.openrouter.api_key`); live default UNCHANGED (Ollama). GATE: ruff + mypy --strict clean; **703 services/server tests / 0 skip** (695 + 8 adapter); NO migration; NO CI change (models/agent already strict). **OR.2 (OpenRouter embeddings) DECIDED AGAINST** (operator, 2026-06-29): reachable (2048-dim, $0) but the shared free-tier daily cap makes a corpus re-embed + per-query embedding impractical — embeddings stay on local nomic-embed (free, uncapped, on-prem). NEXT: **6-e.4** (config_change — the last action class) + the pending **6-e.3 live web-test**.)

_Prior:_ **6-e.3 — rule_tuning: fine-tune existing rules (disable/adjust level) + snapshot-restore reversal.** The third ADR-0025 action class — manager-GLOBAL, Superuser-scoped, reversible. Operator-scoped to: **fine-tune EXISTING rules only** (`disable_rule`→level 0 / `adjust_level`), **`local_rules.xml` only**, **auto-apply** (rules don't hot-reload → the action genuinely applies + verifies). `wazuh/rule_tuning.py` (NEW): the op catalog + **string-based** `local_rules.xml` override construction (multi-root fragment, not ElementTree) — copies the rule's exact inner body from its source file + sets `level` + `overwrite="yes"` so matching conditions are PRESERVED; marked `wolf_tuning` group, idempotent re-tune (no duplicate sid). `capabilities.py`: `ACTION_UPDATE_RULES`/`ACTION_CLUSTER_RESTART` + `rule_tuning`→`{rules:update}` (admin-only — live RBAC probe: `wolf-acme` holds neither, `wazuh-wui` holds both on `*:*:*`). `server_api.py`: read `get_raw` (raw XML, GET-only preserved) + bounded `update_rules_file` (raw PUT `rules:update`) + `restart_cluster` (`cluster:restart`). `executors.py` `_RuleTuningExecutor`: **build_forward** = snapshot `local_rules.xml` (→ `prior_state`) → PUT override → `GET /manager/configuration/validation` (**auto-rollback** the prior file if invalid, never apply a broken ruleset) → cluster restart → verify rule level; **build_reverse** = restore the captured snapshot → validate → restart → tag `REVERSAL_STATE_COMPLETED` → `complete_api_reversal` flips the original to `rolled_back`. **Migration 0017**: `prior_state` JSONB (execute-time bookkeeping, NOT in content_hash; reused by config_change). `validator.py`/`proposals.py`: `_validate_rule_tuning` (rule-id target, op, 0..16 level) + `_rule_tuning_severity` (HIGH — manager-global + reduces visibility). `tools/propose_rule_tuning.py` (NEW, registered): proposes a tune; `restore_rules` undoes an active prior Wolf rule change (linked + recalls why). `prompts.py` #4 + `/actions` rule-target rendering. GATE: ruff + mypy --strict clean; dashboard tsc + eslint clean; **695 services/server tests / 0 skip**; migration 0017 (alembic head `0017`, single head); NO CI change (new modules under already-strict `wazuh`/`tools`/`gateway`; write-boundary grep + alembic-check pass). NEXT: **6-e.4** — config_change (snapshot-restore of ossec.conf, highest blast radius). **LIVE WEB-TEST PENDING** (operator-cleared, Superuser/admin credential — a REAL manager-global write): in Acme as a per-org user, ask to disable a rule → REFUSED (no `rules:update`); as Superuser, disable/adjust a rule → /actions → approve → rule applied (validate→restart→verify); ask to restore the rule → recalls the reason + restores the file + flips the original to rolled_back. Restart wolf-server via `systemctl --user restart wolf-server.service`.)

_Prior:_ **6-e.2 — agent_action: group assign/remove + API-inverse reversal.** The first new action class on the 6-e registry — agent-scoped + reversible, proving the multi-class framework end-to-end. `wazuh/agent_actions.py` (NEW): ops `assign_group`/`remove_group` (mutual inverses) + `is_valid_group`. `capabilities.py`: `ACTION_MODIFY_GROUP` + `agent_action`→`{agent:modify_group}` (Superuser-scoped — per-org creds lack it; capability gate enforces). `server_api.py`: capability-checked `assign_agent_group` (PUT) + `remove_agent_group` (DELETE) on the bounded write client. `validator.py`/`proposals.py`: `_validate_agent_action` + `_agent_action_severity` (medium). `executors.py`: agent_action executor — forward performs the op + **verifies via a fresh group-membership read**; reverse performs the inverse for real, tagging `REVERSAL_STATE_COMPLETED`. `reversal.py`+`api/action_proposals.py`: **`complete_api_reversal`** — a succeeded API-executable reversal flips the ORIGINAL to `rolled_back` + audits (vs AR's wolf-pack-bound reversal, which stays pending). `tools/propose_agent_action.py` (NEW, registered): proposes a group op; the OPPOSITE op for an active prior Wolf action is detected as an **undo** (linked + recalls why). `prompts.py` #4 + `/actions` group-target rendering updated. GATE: ruff + mypy --strict clean; dashboard tsc + eslint clean; **658 services/server tests / 0 skip**; NO migration; NO CI change (new modules under already-strict `wazuh`/`tools`).

_Prior:_ **6-e.1 — framework generalization + ADR 0029 (opens the remaining action classes).** Begins the **6-e** line (`agent_action` → `rule_tuning` → `config_change`, reversal-aware). De-hardcodes the gateway from active-response-only into a **per-class registry**, AR refactored onto it at **zero behaviour change**. **ADR 0029**: per-class scoping (live RBAC probe — agent_action agent-scoped; rule_tuning/config_change manager-GLOBAL → Superuser-scoped, capability-gated) + **two reversal models** (AR wolf-pack-bound vs the new classes' API-inverse / snapshot-restore). Changes: `gateway/validator.py` `validate_proposal` → per-class dispatch (`register_validator`; AR → `_validate_active_response`; shared `require_resolved_agent_target`); `gateway/proposals.py` `compute_severity` per-class (`register_severity`) + `find_active_block` → generalized `find_active_action(matcher)`; `gateway/executors.py` (NEW) per-class executor (`build_forward`/`build_reverse` → `execute_proposal`'s callables, `ExecContext`, `get_executor`/`UnknownActionClassError`); `api/action_proposals.py` `approve` → single executor dispatch (open clients uniformly, pick reverse/forward by `is_reversal`; engine untouched; `stamp_auto_unblock_at` guarded to forward); `wazuh/capabilities.py` `WOLF_ACTION_CLASS_RBAC` → set-per-class. GATE: ruff + mypy --strict clean; **648 services/server tests / 0 skip** (643 baseline + 5 new registry/dispatch/find_active_action tests); AR suite unchanged = zero regression; NO migration; NO CI change (touched modules already strict). NEXT: **6-e.2** — agent_action group assign/remove (the reversible, agent-scoped showcase) + API-inverse reversal + bounded write methods + propose tool + GUI + web-test.)

_Prior:_ **6-d.4 — /actions GUI for reversal + timed blocks (CODE-COMPLETES 6-d).** The browser surface for 6-d (ADR 0028); frontend-only (chat surfacing rode 6-d.2's prompt #4). `lib/types.ts` `ActionProposal` gained `reverses_proposal_id` / `reversal_proposal_id` / `auto_unblock_at` (mirrors the 6-d.2 `ProposalOut`). `app/actions/page.tsx`: a **Reversal / Auto-reversal** sky chip on undo proposals; an **Undoes block #…** field (notes physical removal via wolf-pack); timed blocks show **Duration** ("1h — auto-reverses on expiry"), an executed still-in-effect timed block shows **· auto-reverses <when>**, and a block whose reversal is authorised shows **· reversal authorised**; `resultDetail` surfaces the reversal's honest outcome ("Reversal authorised + recorded — physical removal pending wolf-pack"); the **approve dialog is reversal-aware** — for an undo it says Wolf *authorises + records* the reversal and the host removal runs via wolf-pack (NOT "a real change on your fleet"), so the reviewer is never misled. GATE: dashboard `tsc` + `eslint` clean (dev hot-reloads the page — no build/restart for FE); NO backend change; NO migration; NO CI change (FE rides the existing job). **6-d is code-complete (6-d.1→6-d.4); all CI green on main.** REMAINING: the per-slice **web-test checkpoint** — RESTART wolf-server (stale code until restart: it must pick up the new `unblock_ip`/`enable_user` intents + `block_duration` + `list_active_blocks` tool schema + the auto-reversal scheduler) then exercise on the live cluster: (1) block an IP with a reason → /actions; (2) ask to unblock it → Wolf recalls the reason+evidence; (3) re-block an already-blocked IP → dedup context; (4) a timed block ("block for 2m") → auto-reverses + appears in /actions with "timed block expired" context. **No real host AR without operator go-ahead** (reversals are wolf-pack-bound = no host change; a forward block IS real). NEXT (operator's call): the OTHER action classes (`rule_tuning`/`agent_action`/`config_change`, now reversal-aware via the 6-d linkage), or Phase 6.10 config-settings / 6.7-6.9 notifications+SMTP.)
...
### Infrastructure
- ✅ PostgreSQL 18.4 + pgvector 0.8.5 on `localhost:5432` (PG18 baseline per ADR 0008 addendum; live cluster upgraded 17→18 via `pg_upgradecluster` 2026-07-12, PG17 fully removed)
...
- OS: Ubuntu 24.04 (system PostgreSQL 18.4 + pgvector 0.8.5 via PostgreSQL APT repo; upgraded from 17 on 2026-07-12, PG17 packages removed)
