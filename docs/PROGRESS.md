...
**Last updated:** 2026-06-10 by claude-code (Batch 2 closed; ADRs 0017 + 0018 + 0019 + 0020 PROPOSED; ADR 0018 mid-review with operator — Wazuh-mapping split into 0020 + silent-password-reset rule flipped to allow Superuser reset with audit; memory/ moved into the repo)
...
3. **Four coupled ADRs PROPOSED** (2026-06-10):
   - **ADR 0017 — Wolf Central Brain** (revised v3): the 17-point
     operator requirements list (memory, deep-think, continuous
     learning, self-validation). Language aligned to `organization`
     + `user_id`. New §"Per-organization isolation — non-negotiable"
     made a load-bearing top-level section. New §"MSSP scenario
     worked example" + §"Wazuh access in MSSP" sections. ADR 0021
     idea (separate Organization entity) dropped. v3: Wazuh-mapping
     forward-ref now points to ADR 0020 (split-out).
   - **ADR 0018 — Bootstrap Superuser + Per-Org RBAC + Login UX**
     (revised mid-review with operator): defines the bootstrap flow
     (Wolf ships with Superuser "Wolf", autogenerated 32+ char
     password printed once at install), the per-org role model
     (Superuser / Admin / Engineer / Responder / Analyst — Approver
     renamed to Responder mid-review 2026-06-10), and the
     post-login org-selector UX (no `org_id` at sign-in). Round-1
     review changes: Wazuh component mapping split out to ADR 0020;
     Superuser-can-reset-any-user-password rule flipped from
     forbidden to allowed-with-audit-event. 5 sub-slices: 6.5-a
     through 6.5-e.
