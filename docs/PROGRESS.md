...
**Last updated:** 2026-06-17 by claude-code (**6.6-b.1 — distributed topology refinement** (operator Category-1 UI web-test feedback; refines ADR 0020). Operator web-tested 6.6-b + 6.6-d — ALL Category-1 checkpoints passed. Two change requests, implemented: (1) the required indexer-only `cluster_name` → a uniform **`WazuhNode {url, name?}`** with an OPTIONAL friendly name on EVERY distributed component (UI: Indexer name / Master node name / Worker node name / Dashboard name; blank coerces null); (2) **multiple dashboards** — single `dashboard_url` → a `dashboards` list (≥1), each a probe BLOCKER (workers stay warnings). New distributed shape: `indexer_nodes:[{url,name?}]`(≥1), `manager_master:{url,name?}`, `manager_workers:[{url,name?}]`(0+), `dashboards:[{url,name?}]`(≥1); SINGLE-HOST UNCHANGED. NO migration (topology is JSON, no real data). Backend `wazuh/topology.py` (WazuhNode) + `_probe_topology` (probes every dashboard) + `resolve_endpoints_from_topology` (manager_master.url); frontend topology page distributed builder rewritten with a reusable `NodeList` (url + optional name rows) for indexer/worker/dashboard + a named master. ADR 0020 addendum added. Also clarified for the operator: the topology page's "Saved, but the last probe did not pass." status line is a SEED ARTIFACT (a real hard-fail save always records a passing probe → "Last verified …"); defensive branch kept. Gate: backend ruff + mypy --strict (43 files) + **495 backend / 0 skip / 0 warning** (was 492; +3) + cross-org isolation (18); frontend tsc + eslint(0); live wolf-server clean restart + topology page recompiles + serves 200, PUT 401-gated. No migration, no new dep, NO CI workflow change. **Phase 6.6: 6.6-a/b/b.1/c/d ALL SHIPPED — only 6.6-e (runtime per-query topology+credential resolution, random indexer routing, drop bridged per-org URL columns) remains.** NEXT: operator re-tests the distributed builder (names + multiple dashboards), then build 6.6-e + the Category-2 functional test vs the operator's real Wazuh closes the phase.)
