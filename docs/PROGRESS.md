...
**Last updated:** 2026-06-10 by claude-code (Batch 2 closed; ADRs 0017 + 0018 + 0019 PROPOSED awaiting review; memory/ moved into the repo)
...
3. **Three coupled ADRs PROPOSED** (2026-06-10):
   - **ADR 0017 — Wolf Central Brain** (revised v2): the 17-point
     ...
     idea (separate Organization entity) dropped.
   - **ADR 0018 — Bootstrap Superuser + Per-Org RBAC + Login UX +
     Superuser-owned Wazuh mapping**: defines the bootstrap flow
     (Wolf ships with Superuser "Wolf", autogenerated 32+ char
     password printed once at install), the per-org role model
     (Superuser / Admin / Engineer / Approver / Analyst), the
     post-login org-selector UX (no `org_id` at sign-in), and the
     Superuser-only Wazuh component mapping (both single-host +
     distributed cluster topologies).
   ...
   Phase 12 renamed to wolf-pack + new Phase 6.5 (RBAC + Login +
   Wazuh-mapping per ADR 0018). All three ADRs status: PROPOSED,
