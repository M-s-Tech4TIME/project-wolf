...
**Last updated:** 2026-06-15 by claude-code (6.5-f SHIPPED — Superuser-membership consent gate (ADR 0018), operator web-tested. Install Superuser holds ZERO org data access until an org Admin grants it: Superuser requests (reason + proposed duration, default 24h/until-revoked) → Admin approves (honour/override/until-revoked) or rejects → time-limited UserOrganization row (role superuser) → revoke (Admin) or LAZY expiry (no scheduler — pruned in require_organization_context + the banner endpoint). Migration 0009 (user_organizations.expires_at + superuser_access_requests). Direct-grant precursor REPLACED. Activity timeline: ended_at + revoked/expired terminal statuses stamped on revoke + in expire_if_past; Settings→Access renders per-request lifecycle (Requested→Approved/Rejected/Cancelled→Revoked/Expired w/ actor+time). All-member transparency banner (state-derived, poll+focus+route): FULLY DISMISSABLE per-grant (sessionStorage, returns on new grant/next login). Operator-found + folded in: (a) Superuser chat-nav gate — org-less Superuser bounced off /chat, Chat nav disabled until a grant; (b) MSSP message hygiene — superuser password/role/remove + misconfigured-install rejections no longer leak endpoints/verbs/CLI to tenant Admins (generic "— Unauthorised."/"revoke their access instead"; diagnostic to server log; regression test asserts no /api/·CLI leak). 491 backend + cross-org + mypy --strict green; alembic check clean; frontend tsc/eslint(0)/build green. No CI workflow change needed. DEFERRED to dedicated phases (operator-requested, ADR 0021): per-user notification system (Phase 6.7, poll v1) + SSE real-time push (Phase 6.8), STRICTLY isolated from audit/logs. NEXT: 6.5-h or 6.6 per ordering.)
