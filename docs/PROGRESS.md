...
**Last updated:** 2026-06-16 by claude-code (6.5-h.2 SHIPPED — same-network verification gate (ADR 0018 item 9; topology in ADR 0023). Decisions (operator-approved): TLS edge proxy in front of STOCK Next (not a custom Next server) + gate ON by default with an env escape hatch. Dashboard: new `scripts/edge-proxy.mjs` (Node stdlib) terminates TLS on the public bind, STRIPS client `x-wolf-client-ip`/`x-forwarded-for`/`x-real-ip` + STAMPS the real `socket.remoteAddress` as `X-Wolf-Client-IP` (+ x-forwarded-proto https), forwards to an UNMODIFIED `next dev`/standalone `server.js` on a loopback inner port (Turbopack-dev + standalone-prod untouched); streams responses (SSE preserved) + splices WS upgrades (HMR). Rewired `dev.mjs` (drops --experimental-https; next on inner port + proxy on public bind); prod shim runs the proxy (spawns server.js inner), `debian/wolf-dashboard.install` ships the proxy, unit comments + postinst env hint updated (WOLF_DASHBOARD_TLS_CERT/_KEY, PORT, WOLF_DASHBOARD_INNER_PORT). wolf-server: new `wolf_server/network/local_network.py` (`local_cidrs()` via `ifaddr` + loopback; `client_ip_in_local_network()` fail-closed + v4-mapped-v6 normalise); `verify-invite` resolves the client IP via `_resolve_gate_client_ip` — trusts `X-Wolf-Client-IP` ONLY when mTLS-authenticated as the dashboard (request.state.mtls_cert_cn), else the real TCP peer (un-spoofable) — and CIDR-checks it; out-of-network → 403 `wrong_network` WITHOUT consuming the token. Config flag `same_network_gate_enabled` (default FALSE — MSSP-safe; `SAME_NETWORK_GATE_ENABLED=1` enables on-prem single-network deploys); startup banner prints state. `/verify` page gains a network hint. Tests: `test_local_network.py` (enumeration, membership, normalise, fail-closed, trust rule) + 3 gate integration tests in test_rbac.py (off-network 403 + token kept, gate-off allows any, spoofed header ignored w/o mTLS); `ifaddr>=0.2` + uv lock. CI: `wolf_server/network` added to mypy strict-set (ci.yml + Makefile); smoke-deb-install asserts edge-proxy.mjs ships. Gate green: ruff + mypy --strict + 449 backend / 72 package tests (0 skip) + cross-org isolation + dep-audit (ifaddr clean); frontend tsc/eslint(0)/build (standalone server.js present); edge proxy strip/stamp + no-buffering validated in isolation; live HTTPS :3000 + login POST→401 over mTLS confirmed; same-network banner ENABLED. MSSP GAP surfaced mid-slice (operator): the gate checks membership in *wolf-server's* network, so a default-ON gate permanently blocks remote MSSP client orgs from verifying. Decision (operator-approved): flip default ON→OFF (MSSP-safe; on-prem opts in), ship the gate as inert-but-ready machinery, commit now. Follow-ups recorded: **Phase 6.10 — Superuser config-settings system** (synced web/CLI/env + Settings GUI, Superuser-only, ADR 0019) turns the gate into a GUI toggle; **per-org trusted networks** = the MSSP-correct gate. Standing rule captured: ALL Wolf management/config is Superuser-only; other settings role-scoped (memory `superuser-config-authority`). NOTE: torch (opt-in embeddings-local extra) has CVE-2025-3000, no fix yet — out of CI dep-audit scope (not in default .deb). NEXT: 6.6 (Superuser Wazuh mapping) / Phase 6.10 (config-settings) / Phase 6.9 (SMTP) per operator.)
