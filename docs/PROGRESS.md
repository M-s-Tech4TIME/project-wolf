...
**Last updated:** 2026-06-17 by claude-code (**6.6-d SHIPPED** — per-org Wazuh Credentials UI + rotation log (ADR 0020), the GUI for the 6.6-c backend. New `components/wazuh-credentials-card.tsx` rendered on each org's Superuser detail page (`app/superuser/organizations/[id]/page.tsx`): indexer + Server-API user/password fields (write-only — usernames shown, blank password = keep existing, "•••• (unchanged)" placeholder), index filter, optional comma-separated agent groups, inject-organization-filter toggle. "Test & save" is SOFT-FAIL — saves even when the probe fails (configure before the Wazuh-side user exists), rendering per-endpoint probe results (✓/✗) + the SCOPE SUMMARY ("credential sees N agents…") + warnings + a verified/not-yet-verified status. A 409 (no install topology yet) renders a guided alert linking to `/superuser/wazuh`. Client-side validation mirrors the backend. Small NEW Superuser endpoint `GET /api/v1/superuser/organizations/{id}/wazuh-credentials/history` (ROTATION LOG — org-scoped `organization.wazuh_credentials.*` audit projection via typed extractors, NEVER returns creds) + 2 tests; the install-wide audit view can't scope to one org cleanly, hence this focused read. New `lib/types.ts` per-org creds types + `lib/api.ts` fetchOrgWazuhCredentials/saveOrgWazuhCredentials/fetchOrgWazuhCredentialHistory. Gate: backend ruff + mypy --strict (43 files) + **492 backend / 0 skip / 0 warning** (was 490; +2 history) + cross-org isolation (18); frontend tsc + eslint(0); live per-org route compiles + serves 200, history endpoint 401 unauth through proxy. No migration (no schema change), no new dep, NO CI workflow change. **Phase 6.6: backends (6.6-a, 6.6-c) + UIs (6.6-b, 6.6-d) ALL SHIPPED — only 6.6-e (runtime per-query topology+credential resolution, random indexer routing, drop the bridged per-org URL columns) remains.** WEB-TEST PLAN (operator 2026-06-17, memory `phase-6.6-web-test-plan`): Category-1 UI test (gating/builders/validation/hard+soft-fail, NO Wazuh) = ONE consolidated pass over 6.6-b + 6.6-d, to run NEXT (I prep a clean state + hand off); Category-2 functional test runs FOR REAL at 6.6-e against the operator's Wazuh (operator HAS one). NEXT: consolidated 6.6-b+6.6-d UI web-test, then build 6.6-e.)
