...
**Last updated:** 2026-06-16 by claude-code (**Phase 6.6 OPENED — 6.6-a SHIPPED**: install-level Wazuh ecosystem topology, backend only (ADR 0020). Phase 6.6 = Superuser-owned Wazuh component mapping, 5 sub-slices. Proceeding NOW ahead of Phase 6 (wolf-gateway) per operator direction — 6.6 depends only on shipped 6.5 Superuser+RBAC, not on the gateway; Phase 6 stays designed-not-started + unblocked. 6.6-a delivers the INSTALL-level layer (where the indexer(s)/manager(s)/dashboard physically live); the per-org *credentials* layer is the separate 6.6-c refactor of `organization_wazuh_configs`. New `wolf_server/wazuh/probe.py` (reusable `probe_indexer`/`probe_manager_api`/`probe_dashboard` → `EndpointProbeResult`; never raises on HTTP/transport failure — caller decides hard-vs-soft; shapes lifted from `bootstrap_organization._validate_wazuh_connection`; re-used by 6.6-c). New `wolf_server/wazuh/topology.py` (pydantic discriminated union SingleHost/Distributed on `kind` — single source of truth for shape validation, shared by API + persisted JSON; http(s)+length validation; distributed needs ≥1 indexer node). New `WazuhEcosystemTopology` ORM + migration 0011 (single-row install-wide, DB-enforced singleton via unique `is_singleton`; stores kind + topology JSONB + credential KEYS + verify_tls + validated_at; passwords → secrets backend ONLY, ADR decision 7). New `wolf_server/api/wazuh_topology.py` router: `GET`/`PUT /api/v1/superuser/wazuh-topology` (Superuser-only via `require_superuser`; ADR path, NOT the roadmap's stale `/api/v1/install/...` — roadmap corrected). PUT = validate-before-persist HARD fail (ADR decision 3): probe every endpoint, REJECT save if any BLOCKER fails (indexer node(s)+manager master+dashboard); distributed manager WORKERS are warnings, not blockers. Audit `install.wazuh_topology.updated`/`…probe_failed` (system-level, NEVER logs creds). "Omit password ⇒ keep existing credential" (422 if omitted on first save). Router mounted in main.py. INERT at runtime this slice (query path still uses per-org config; wired into runtime resolution at 6.6-e). Gate: ruff + mypy --strict (`wolf_server/wazuh` already in strict set — no CI change) + **476 backend / 0 skip / 0 warning** (was 449; +27 — 13 probe unit via httpx.MockTransport, 14 model+API) + cross-org isolation (18); `alembic check` clean ("No new upgrade operations detected") + 0011 round-trips on Postgres; no new dep (dep-audit unaffected). Live: wolf-server restarted clean (`wolf_server_ready`), GET+PUT 401 unauth through the real dashboard proxy + mTLS path (mounted + Superuser-gated). NO CI workflow change needed (strict-mypy set already covers, alembic-check covers 0011, no new shipped file for smoke-deb). DEFERRED operator web-test (tracked, NOT skipped): full functional test — configure a real Wazuh ecosystem via the GUI + live endpoint probes — lands with 6.6-b (UI)/6.6-e (runtime), needs real Wazuh. NOTE retained: torch (opt-in embeddings-local extra) CVE-2025-3000, no fix — out of CI dep-audit scope. NEXT: 6.6-b (install-level Wazuh Ecosystem UI) / 6.6-c (per-org creds refactor) per operator.)
