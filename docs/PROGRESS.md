...
**Last updated:** 2026-06-17 by claude-code (**6.6-e SHIPPED — runtime per-query topology + credential resolution (ADR 0020); Phase 6.6 FEATURE-COMPLETE.** The last build slice wires the two Wazuh backends + two UIs into the live query path. `resolver.get_wazuh_connection` rewired: URLs + TLS posture now come from the install ecosystem topology (6.6-a), read FRESH PER QUERY (single → the one indexer/manager; distributed → a RANDOM indexer node per ADR 0020 decision 1 + the manager master); only the per-org CREDENTIALS + index filter + organization-filter flag come from `organization_wazuh_configs` (6.6-c). Signature unchanged → `chat.py` + clients untouched. New `WazuhTopologyMissingError` (404, a WolfError surfaced by the global handler like the existing WazuhConfigMissingError) when no topology configured — you MUST configure the ecosystem before any org can query Wazuh (intended ADR 0020 migration). New `tests/test_resolver.py` (4) proves topology URLs + verify_tls override the STALE per-org URL columns, per-org index filter + creds + org-filter flag are used, distributed picks a real node (sampled), and both missing-state errors fire. VESTIGIAL: the per-org URL columns (opensearch_url/server_api_url/verify_tls) are NO LONGER READ (still written by bootstrap CLI + 6.6-c API to satisfy NOT-NULL) — tracked follow-up: drop them + modernise `bootstrap_organization`'s --*-url args + add indexer-node FALLBACK-on-failure (random selection ships now; retry-other-nodes needs a multi-node cluster to exercise). Gate: ruff + mypy --strict (43 files) + **499 backend / 0 skip / 0 warning** (was 495; +4 resolver) + cross-org isolation (18); no migration, no new dep; wolf-server restarts clean with the rewired resolver. NO CI workflow change. **Phase 6.6 (6.6-a/b/b.1/c/d/e) ALL SHIPPED + CI-green** — exit criteria met PENDING the operator's CATEGORY-2 functional web-test against a REAL Wazuh (configure topology → per-org creds → an org user chats → Wolf queries that org's Wazuh data). NEXT: prep + hand off that functional web-test (memory `phase-6.6-web-test-plan`; operator HAS a Wazuh); then per operator — Phase 6 (wolf-gateway) / 6.10 (config-settings) / 6.9 (SMTP).)
