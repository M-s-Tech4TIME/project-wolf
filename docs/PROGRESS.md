...
**Last updated:** 2026-06-16 by claude-code (**6.6-b SHIPPED** — install-level Wazuh Ecosystem UI (ADR 0020), the Superuser GUI for the topology shipped by 6.6-a. Frontend-only. New `app/superuser/wazuh/page.tsx` (Superuser-only, reached from a new "Wazuh ecosystem" nav item in `app/superuser/layout.tsx`): Single/Distributed segmented builder — single = indexer/manager/dashboard URLs; distributed = dynamic indexer-node list (url + cluster_name, add/remove) + manager master URL + dynamic worker list (add/remove; "failed worker is a warning, not a blocker") + dashboard URL. Write-only credentials (indexer-admin + manager-API usernames shown; password fields blank with "•••• (unchanged)" placeholder when configured → blank means keep stored secret); verify-TLS checkbox. "Test & save" → PUT: on the backend's validate-before-persist HARD-fail 400 the guided `detail` (which endpoints failed) renders in a destructive alert + nothing saved; on success a per-endpoint probe-result list (✓/✗ + detail) + worker warnings + "last verified <relative time>" render. Client-side validation mirrors the backend (http(s) scheme, required usernames, first-save passwords). New `lib/types.ts` Wazuh-topology types + `lib/api.ts` `fetchWazuhTopology`/`saveWazuhTopology` (Superuser-only, org-less → no X-Organization-Id). Gate: `tsc --noEmit` clean + `eslint .` 0 warn/err (fixed a `react-hooks/set-state-in-effect` by dropping a redundant synchronous setLoading(true)); live dev route `/superuser/wazuh` compiles + serves 200 through the real proxy; called endpoints return 401 unauth. NO `npm run build` locally (CI `frontend` job runs it — memory next-dev-cache-vs-build); no backend change, no new dep, NO CI workflow change needed. DEFERRED operator web-test (tracked, NOT skipped): live end-to-end (log in as Superuser, fill the form against a REAL Wazuh, observe hard-fail vs success + probe results) lands with 6.6-e (runtime) + real Wazuh. Phase 6.6 status: backend layers (6.6-a topology, 6.6-c per-org creds) DONE; UIs — 6.6-b install topology DONE, 6.6-d per-org creds UI NEXT; then 6.6-e runtime resolution closes the phase. NEXT: 6.6-d (per-org creds UI) / 6.6-e (runtime resolution) per operator.)
