...
| 0029 | `additional-action-classes.md` — Phase 6-e: the remaining action classes (agent_action → rule_tuning → config_change) on the same propose→approve→execute pattern. Per-class scoping (agent-scoped vs manager-global/Superuser-scoped, capability-gated) + **two reversal models** (AR wolf-pack-bound vs API-inverse / snapshot-restore). Generalizes the gateway into a per-class registry (validator / severity / find_active_action / executor / capability-set); AR refactored onto it, zero behaviour change. | accepted |
| 0030 | `openrouter-model-provider.md` — OpenRouter as a hosted model provider behind the existing provider abstraction; API key via the secrets backend (`model.openrouter.api_key`); graded `KNOWN_MODELS` entries; local Ollama remains the private/uncapped default. | accepted |
| 0031 | `provider-failover-chain.md` — `FailoverProvider` chain (primary → fallback) failing over on any non-cancel error, per-instance breaker, conservative capability floor; wired via `FALLBACK_MODEL_*`. Addenda: OpenRouter free reality (50 req/day per ACCOUNT across all `:free` models) + reactive quota visibility from 429/402 headers; graceful SSE degradation on any model failure. | accepted |
| 0032 | `web-research-and-config-authoring.md` — web research as a universal power: provider-agnostic `web_search`/`web_fetch`/bounded `web_crawl` (SSRF guard, untrusted-content envelope, budgets, citations), SearXNG self-hosted default (`wolf-search`), docs-first re-rank; config-authoring generalization (block-identity upsert/remove, two-phase confirm-diff, research-to-act). Addenda: no hard step caps / unbounded persistence; any-unique-field block disambiguation; deployment-aware (per-cluster-node) config application. | accepted |
| 0033 | `fully-configurable-embedding-stack.md` — every embedding knob per-embedder (dimension, MRL request-dimensions, query/document prefixes, num_ctx, char limits); pgvector columns follow settings via resumable `embedding_schema --apply`. Addenda: **no dimension cap** — >2000 dims index via binary-quantized HNSW + exact-cosine rerank; `embedding_bench` verdict — qwen3-embedding native-4096 + v2-moe combo is the measured best on Wolf's corpus (MRR@10 0.963 vs 0.766 nomic combo). | accepted |

_Update this table whenever you add a new ADR._
_(Index previously trailed 0017–0020 — still a pre-existing gap; 0024–0028 backfilled 2026-06-28; 0030–0033 backfilled at the 2026-07-12 wrap-up.)_
