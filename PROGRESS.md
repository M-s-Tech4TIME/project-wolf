...
| **5.0c** | UI overhaul: persistent + resizable + text-wrapping Evidence panel · collapsible Conversations sidebar · fixed message input · chat vertical scroll · user-avatar dropdown replacing the session-id chip. **Plus:** (a) **verdict rename + 4 chips** — `supported`→🟢 *Verified*, `unverifiable`→🟡 *Non-factual* (muted), `uncertain`→🟡 *Uncertain* (amber), `unsupported`→🔴 *Not Verified* (needs backend marker emission for the two new verdicts + frontend chip styles); (b) **progressive answer rendering** — Claude-style token-by-token reveal so the answer feels alive instead of arriving as a 3-minute wall; (c) **live activity feed during steps** — dynamic, natural-language status messages that track what Wolf is actually doing right now (calling Wazuh, reading N alerts, asking the model, judging with the validator, drafting the answer, etc.) — see [[progressive-response-and-live-activity]]. | ⏳ not started | — |
